...
(function () {
  ...
  const MANIFEST_URL  = "https://assets.northaventrail.org/json/trail-captains.latest.json";
  const SIGNUP_EMAIL  = "trailcaptainsignup@northaventrail.org";
  const SIGNUP_SUBJECT = "I would like to learn more about being a trail captain";
  ...
  function renderHeader(headerEl, sections, sectionOrder) {
    ...
    const navLinks = sectionOrder.map(function (key) {
  ...
  function renderTable(container, sectionData) {
    ...
    const vacancyBadge = vacantCount > 0
    ...
    fetchJson(MANIFEST_URL, "no-store", controller.signal)
      ...
      .then(function (data) {
        ...
        containers.forEach(function (el) {
          ...
          let anchorTarget  = el;
          let blockAncestor = el.parentElement;
          ...
          if (blockAncestor) {
            ...
            while (prev) {
              const h2 = prev.tagName === "H2" ? prev : prev.querySelector("h2");
              if (h2) {
                anchorTarget = h2;
                break;
              }

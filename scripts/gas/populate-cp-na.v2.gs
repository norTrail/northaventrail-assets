 ...
 * - Display lives in the `a` column/key and only truthy rows are eligible.
 * - Force Override may only exist as a row-3 title and not as a row-4 JSON key.
 */
...
function populateCpAndNaForPoiSheet(options) {
  return populateCpAndNaForSheet(SHEET_POI_SLIM, options);
}
...
function dryRunPopulateCpAndNaForPoiSheet(options) {
  var merged = mergeOptions_(DEFAULT_CP_NA_OPTIONS, options || {});
  merged.dryRun = true;
  return populateCpAndNaForPoiSheet(merged);
}

function testPopulateCpAndNaFirstTwoRows() {
  return populateCpAndNaForPoiSheet({
    maxRowsToWrite: 2,
    overwriteCp: true,
    overwriteNa: true
  });
...
function dryRunTestPopulateCpAndNaFirstTwoRows() {
  return populateCpAndNaForPoiSheet({
    maxRowsToWrite: 2,
    overwriteCp: true,
    overwriteNa: true,
...
function populateCpAndNaForSheet(sheetName, options) {
  ...
  var columnRefs = resolveColumnRefs_(headerMap, headerRowValues, settings);
  ...
  var outputArrays = buildOutputArrays_(parsed, results, settings);
...
function resolveColumnRefs_(headerMap, headerRowValues, options) {
  return {
...
function findColumnByHeaderTitle_(headerRowValues, title) {
  var normalizedTitle = normalizeHeader_(title);
  for (var i = 0; i < headerRowValues.length; i += 1) {
    if (normalizeHeader_(headerRowValues[i]) === normalizedTitle) {
      return i + 1;
    }
  }
  return null;
  ...
  for (var rowIndex = 0; rowIndex < values.length; rowIndex += 1) {
    ...
    if (validationErrors.length || !isActive) {
      invalidRows.push({
        rowNumber: rowNumber,
        id: id,
        reasons: !isActive ? validationErrors.concat(['not displayed']) : validationErrors
      });
      continue;
    }
...
function isRowActive_(rawRow, columnRefs, options) {
  ...
  return isTruthyCell_(getCell_(rawRow, columnRefs.active));
}
...
function buildOutputArrays_(parsed, results, options) {
  var cpValues = [];
  var naValues = [];
  var debugStatus = [];
  var debugCpDistance = [];
  ...
  for (var rowIndex = 0; rowIndex <= maxRowIndex; rowIndex += 1) {
    ...
    var rowStatus = [];
    var shouldWriteRow = options.maxRowsToWrite == null || writtenFeatureRows < options.maxRowsToWrite;

    if (locked) {
      lockedRowsSkipped += 1;
      rowStatus.push('locked');
    } else if (!shouldWriteRow) {
      rowStatus.push('outside-test-write-limit');
    } else {
